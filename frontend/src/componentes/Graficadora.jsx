...
function Graficadora({ usuario }) {
    ...
    useEffect(() => {
        fetch('http://localhost:8080/backend/MostrarDiagramas')
            .then(r => r.json())
            .then(data => setDiagramas(Array.isArray(data) ? data : []))
            .catch(() => {});
    }, []);
            ...
            <div className="dash-body">
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 28, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    >
                        ...
                        {diagramas.map(d => (
                            <option key={d.id} value={d.id} style={{ background: '#0f1e36' }}>
                                #{d.id} — {d.nombre}
                            </option>
                        ))}
                    </select>
                    ...
                    <button
                        ...
                        onClick={() => cargarDiagrama(idSeleccionado)}
                        disabled={!idSeleccionado || cargando}
                    >
                ...
                <div className="grafica-panel">
                    ...
                    <div style={{ marginTop: 16, display: 'flex', gap: 16, flexWrap: 'wrap', fontSize: '0.75rem', color: 'rgba(248,250,255,0.45)' }}>
                        {[
                            ...
                            { color: '#2563eb', label: 'Proceso' },
                            { color: '#f59e0b', label: 'Decisión' },
                            { color: '#8b5cf6', label: 'Entrada/Salida' },
